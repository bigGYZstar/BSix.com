 ...
 * タイムラインコンポーネント
 */
function createTimelineComponent(events: TimelineEvent[], fixture: Fixture): HTMLElement {
 ...
 * 個別タイムラインイベント
 */
function createTimelineEvent(event: TimelineEvent, fixture: Fixture): HTMLElement {
...
function createEventIcon(event: TimelineEvent): HTMLElement {
  ...
  icon.className = 'flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-white text-xs font-bold z-10'
 ...
 * イベントアイコンスタイル
 */
function getEventIconStyle(event: TimelineEvent): { color: string; symbol: string } {
  if (event.type) {
    const styles = {
...
function analyzeTimelineStats(timeline: TimelineEvent[]): {
  ...
  timeline.forEach(event => {
    ...
    if (event.type === 'substitution') stats.substitutions++
    if (event.type === 'card') stats.cards++
  })
 ...
 * タイムライン フィルタリング機能
 */
export function addTimelineFilters(container: HTMLElement, fixture: Fixture): void {
