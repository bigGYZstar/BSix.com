...
function createPitchSVG(): SVGElement {
  ...
  // ペナルティエリア（上）
  const penaltyTop = document.createElementNS('http://www.w3.org/2000/svg', 'rect')
  ...
  // ペナルティエリア（下）
  const penaltyBottom = document.createElementNS('http://www.w3.org/2000/svg', 'rect')
  ...
  // ゴールエリア（下）
  const goalBottom = document.createElementNS('http://www.w3.org/2000/svg', 'rect')
 ...
 * 選手ノード配置
 */
function createPlayerNodes(formationData: FormationData, teamId: string): HTMLElement[] {
...
export function adjustPlayerNodeSize(container: HTMLElement): void {
  ...
  const nameboxes = container.querySelectorAll('.namebox') as NodeListOf<HTMLElement>
