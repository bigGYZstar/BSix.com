...
test.describe('Team Pills No-Wrap Test', () => {
  ...
  VIEWPORT_SIZES.forEach(({ width, height }) => {
    test(`should not wrap team pills at ${width}x${height}`, async ({ page }) => {
      // ビューポートを設定
  ...
  test('should maintain single line even with long team names', async ({ page }) => {
  ...
  test('should be horizontally scrollable on narrow screens', async ({ page }) => {
    await page.setViewportSize({ width: 320, height: 568 })
  ...
  test('should handle theme switching without breaking layout', async ({ page }) => {
